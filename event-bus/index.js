...
app.post('/events', async (req, res) => {
    ...
    await axios.post('http://auth-service:4000/events', event).catch((err) => {
        console.log('Auth Service: ', err.message);
    });

    await axios.post('http://classroom-service:4001/events', event).catch((err) => {
        console.log('Classroom Service: ', err.message);
    });

    await axios.post('http://post-service:4002/events', event).catch((err) => {
